...
import com.ericsson.eniq.etl.RedbackParser.NameValuePair;
import com.ericsson.eniq.etl.RedbackParser.RedbackParser;
...
//      getRowCount = RedbackParser.class.getDeclaredMethod("getRowCount", null);
 ...
 //     m_dataStorePolicy = RedbackParser.class.getDeclaredField("m_dataStorePolicy");
  ...
  public void testStorageOfFirstRow(){
	  RedbackParser ap = new RedbackParser();
	  try {
		  groupKeysField.set(ap, this.EPS_APN_POLICY_KEYSField.get(ap));
  ...
  public void testStorageOfSecondRowWithSameKey(){
	  ...
	  try {
		  ...
		  assertEquals(1, map.size());
		  assertEquals(6, map.get("value2").size());
	  } catch (IllegalArgumentException e) {
		  fail(e.getMessage());
	  } catch (IllegalAccessException e) {
		  fail(e.getMessage());
  ...
  public void testStorageOfSecondRowWithDiffKey(){
	  ...
	  try {
		//Set the policy (it determines how may group keys there are)
		  ...
		  assertEquals(4, map.get("value2").size());
  ...
  public void checkGroupKeys(){
	  ...
	  try {
		  ...
		  groupKeysField.set(ap, this.CHANNEL_POLICY_KEYSField.get(ap));
  ...
  public void checkGroupKeys2(){
	  ...
	  try {
		  ...
		  groupKeysField.set(ap, this.CHANNEL_POLICY_KEYSField.get(ap)); //This policy has 4 group keys
		boolean result = (Boolean)checkGroupKeysMethod.invoke(ap, list1);
		assertTrue(result);
	} catch (IllegalArgumentException e) {
		  fail(e.getMessage());
  ...
  public void checkGroupKeys3(){
	  ...
	  try {
		  ...
		  list1.add(new NameValuePair("slot", "value2"));
		  list1.add(new NameValuePair("port", "value3"));
		  groupKeysField.set(ap, this.CHANNEL_POLICY_KEYSField.get(ap));//This policy has 4 group keys, more then are in list1.
		boolean result = (Boolean)checkGroupKeysMethod.invoke(ap, list1);
		assertFalse(result);
	} catch (IllegalArgumentException e) {
		  fail(e.getMessage());
